...
--Automatically indexed by phone_number, no need to explicitly declare an index to 
--get better performance
create table unsubscribed_phone_numbers (
    phone_number text unique primary key,
    unsubscribed_profiles uuid[] default '{}'
);
--if the table delivery_reports had already been committed and had data, we would need 
--a step to populate unsubscribed_phone_numbers but I'm leaving that out for this assignment
...
as $$
    declare
        ...
        loop_id uuid;
    begin
        select to_number, profile_id 
            into new_phone_number, new_profile_id
